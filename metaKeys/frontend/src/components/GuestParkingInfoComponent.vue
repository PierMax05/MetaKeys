<template>
  <div class="guest-parking-info">
    ...
    <div v-else>
      <div v-if="parkingInfos.length">
        <div class="info-type">
          <h5>Informazioni sul Parcheggio</h5>
        </div>
        <div class="info-cards">
          <div class="info-cards-container">
            <div v-for="(info, index) in parkingInfos" :key="info.id" 
                 :class="['info-card', { 'active': activeCard === info.id, 'first-card': index === 0, 'last-card': index === parkingInfos.length - 1, 'scrollable': activeCard === info.id }]"
                 @click="setActiveCard(info.id, $event)">
              <div :class="['card-content', { 'scrollable': activeCard === info.id }]">
                <h4>{{ info.title }}</h4>
                <p>{{ info.info }}</p>
              </div>
              <div v-if="info.google_link">
                <br>
      ...
      <div v-else>
        <p>Nessuna informazione sul parcheggio disponibile.</p>
      </div>
    </div>
  </div>
</template>
...
export default {
  name: 'GuestParkingInfoComponent',
  data() {
    return {
      loading: true,
      activeCard: null,
  ...
  computed: {
    ...
    parkingInfos() {
      return this.infos.filter(info => info.type === 'parking_info');
    },
  },
  methods: {
