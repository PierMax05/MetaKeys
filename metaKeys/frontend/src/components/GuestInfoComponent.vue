        ...
        <div class="info-cards">
          <div class="info-cards-container">
            <div v-for="(info, index) in generalInfos" :key="info.id" 
                 :class="['info-card', { 'active': activeCard === info.id, 'first-card': index === 0, 'last-card': index === generalInfos.length - 1, 'scrollable': activeCard === info.id }]"
                 @click="setActiveCard(info.id, $event)">
              <div :class="['card-content', { 'scrollable': activeCard === info.id }]">
                <h4>{{ info.title }}</h4>
                <p>{{ info.info }}</p>
              </div>
              <div class="card-footer" v-if="info.google_link && info.google_link.trim() !== ''">
                <br>
                <div>
        ...
        <div class="info-cards">
          <div class="info-cards-container">
            <div v-for="(info, index) in nearbyInfos" :key="info.id" 
                 :class="['info-card', { 'active': activeCard === info.id, 'first-card': index === 0, 'last-card': index === nearbyInfos.length - 1, 'scrollable': activeCard === info.id }]"
                 @click="setActiveCard(info.id, $event)">
              <div :class="['card-content', { 'scrollable': activeCard === info.id }]">
                <h4>{{ info.title }}</h4>
                <p>{{ info.info }}</p>
        ...
        <div class="info-cards">
          <div class="info-cards-container">
            <div v-for="(info, index) in placesToVisitInfos" :key="info.id" 
                 :class="['info-card', { 'active': activeCard === info.id, 'first-card': index === 0, 'last-card': index === placesToVisitInfos.length - 1, 'scrollable': activeCard === info.id }]"
                 @click="setActiveCard(info.id, $event)">
              <div :class="['card-content', { 'scrollable': activeCard === info.id }]">
                <h4>{{ info.title }}</h4>
                <p>{{ info.info }}</p>
